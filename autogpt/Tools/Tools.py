...
@tool("Calendar")
def calendar_tool(date_exp: str = Field(description="Date expression to be parsed. It must be in English."),) -> str:
    """用于查询和计算日期/时间"""
    try:
        res = ctparse(date_exp)
        if res is None or res.resolution is None or res.resolution.dt is None:
            return "无法解析该日期表达式，请使用更明确的英文描述。"
        
        dt = res.resolution.dt
        #ret = dt.strftime("%c")  # 或更清晰的格式如 "%Y-%m-%d %H:%M:%S"
        ret = dt.strftime("%Y-%m-%d %H:%M:%S")  # 或更清晰的格式如 "%Y-%m-%d %H:%M:%S"
        ret = "{'content':'"+ret+"'}"
        print(ret)
        return ret
    
    except Exception as e:
        return f"日期解析出错: {str(e)}"
